...
function getOracleNodeAndLabels() {
    var oracleData = [{
        index: 16,
        label: 3
    }, {
...
function getOracleNodeAndLabels2() {
    var oracleData = [{
        index: 19,
        label: 3
    }, {
...
function getOracleNodeAndLabels3() {
    var oracleData = [{
        index: 17,
        label: 3
    }, {
...
function getOracleNodeAndLabels4() {
    var oracleData = [{
        index: 19,
        label: 3
    }, {
...
function getOracleNodeAndLabels5() {
    var oracleData = [{
        index: 1,
        label: 1
    }, {
...
var force = d3.layout.force()
    .nodes(graph.nodes, function(d) {
        return d.id;
    })
    .links(graph.links, function(d) {
        return d.id;
    })
    .size([width, height])
    .linkDistance([50])
...
**/
setTimeout(function() {
    var oracleData = getOracleNodeAndLabels();
    //setting nodes chosen by oracle as mod = true
    for (var i = 0; i < oracleData.length; ++i) {
        graph.nodes[oracleData[i].index].class = oracleData[i].label;
        graph.nodes[oracleData[i].index].mod = true;
    }

    nodes.filter(function(d) {
            return d.mod;
        })
        .classed('oracle-select', true)
        .transition()
        .duration(transitionDuration)
        .attr('r', radius + 5)
        .transition()
